...
static uint16_t measurement_idx = 0;

static uint64_t start[SAMPLES];
static uint64_t finish[SAMPLES];
...
int main(void)
{
    ...
    if (xMutex != NULL)
    {
        // 2. Tworzenie zadań (Priorytet musi być taki sam, np. osPriorityNormal)
        xTaskCreate(Task_A, "TaskA", 128 * 4, NULL, osPriorityNormal, &xTaskAHandle);
        xTaskCreate(Task_B, "TaskB", 128 * 4, NULL, osPriorityNormal, &xTaskBHandle);
    }
