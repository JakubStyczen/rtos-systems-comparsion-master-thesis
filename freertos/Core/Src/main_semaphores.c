...
int main(void)
{
    ...
    dwt_init();
    sem_100 = xSemaphoreCreateBinary();
    sem_200 = xSemaphoreCreateBinary();
    sem_500 = xSemaphoreCreateBinary();
    sem_1000 = xSemaphoreCreateBinary();
    sem_5000 = xSemaphoreCreateBinary();
    sem_10000 = xSemaphoreCreateBinary();

    xTaskCreate(Task100, "Task100", 512, t_100, osPriorityNormal, NULL);
    //  xTaskCreate(Task200, "Task200", 512, t_200, osPriorityNormal, NULL);
    //  xTaskCreate(Task500, "Task500", 512, t_500, osPriorityNormal, NULL);
    //  xTaskCreate(Task1000, "Task1000", 512, t_1000, osPriorityNormal, NULL);
    //  xTaskCreate(Task5000, "Task5000", 512, t_5000, osPriorityNormal, NULL);
    //  xTaskCreate(Task10000, "Task10000", 512, t_10000, osPriorityNormal, NULL);
    ...
    TimerHandle_t t100 = xTimerCreate("t100", 1, pdTRUE, NULL, cb_100);
...
void StartTimerTest(void const *argument)
{
    ...
    {
        //    	 printf("wchodze");
        if (done_10000) // albo AND wszystkich
        {
            printf("=== SERIES %lu ===\n", repeat + 1);
