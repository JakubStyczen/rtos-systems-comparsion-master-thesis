...
void create_semaphores()
{
    ...
    status = xTaskCreate(task100, "t100", 128, NULL, 6, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task100\r\n");
    }
    status = xTaskCreate(task200, "t200", 128, NULL, 5, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task200\r\n");
    }
    status = xTaskCreate(task500, "t500", 128, NULL, 4, NULL);
    if (status != pdPASS)
    {
    ...
    status = xTaskCreate(task5000, "t5000", 128, NULL, 2, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task5000\r\n");
    }
    status = xTaskCreate(task10000, "t10000", 128, NULL, 1, NULL);
    if (status != pdPASS)
    {
