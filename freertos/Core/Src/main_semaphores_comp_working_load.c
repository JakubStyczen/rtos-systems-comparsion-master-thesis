...
volatile int c5000 = 0;
volatile int c10000 = 0;

static void reset_counters(void)
{
    c100 = c200 = c500 = c1000 = c5000 = c10000 = 0;
    //    tick = 0;
}
...
void create_semaphores()
{
    ...
    status = xTaskCreate(task100, "t100", 512, NULL, 6, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task100\r\n");
    }
    status = xTaskCreate(task200, "t200", 512, NULL, 5, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task200\r\n");
    }
    status = xTaskCreate(task500, "t500", 512, NULL, 4, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task500\r\n");
    }
    status = xTaskCreate(task1000, "t1000", 512, NULL, 3, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task1000\r\n");
    }
    status = xTaskCreate(task5000, "t5000", 512, NULL, 2, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task5000\r\n");
    }
    status = xTaskCreate(task10000, "t10000", 512, NULL, 1, NULL);
    if (status != pdPASS)
    {
        printf("BŁĄD: Nie udalo sie utworzyc task10000\r\n");
    }
}
