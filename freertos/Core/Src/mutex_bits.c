...
#define BIT_READY_FOR_T1 (1 << 0) // Task1 może zacząć
#define BIT_READY_FOR_T2 (1 << 1) // Task2 może zacząć
...
void task1(void *arg)
{
    for (;;)
    {
        // Czekamy na bit 0 (T1 Ready). pdTRUE = wyczyść bit po odczycie.
        xEventGroupWaitBits(xEventGroup, BIT_READY_FOR_T1, pdTRUE, pdTRUE, portMAX_DELAY);
        ...
        printf("Task1");
        // Aktywuj Task 2
        xEventGroupSetBits(xEventGroup, BIT_READY_FOR_T2);
    }
    vTaskDelete(NULL);
}
...
void task2(void *arg)
{
    for (;;)
    {
        // Czekamy na bit 1 (T2 Ready)
        xEventGroupWaitBits(xEventGroup, BIT_READY_FOR_T2, pdTRUE, pdTRUE, portMAX_DELAY);
        printf("Task2");
        if (count < SAMPLES)
        {
        ...
        // Pozwól Task 1 na kolejny krok
        xEventGroupSetBits(xEventGroup, BIT_READY_FOR_T1);
    }
    vTaskDelete(NULL);
}
...
int main(void)
{
    ...
    // Ustawiamy bit dla Task1, żeby mógł wystartować jako pierwszy
    if (xEventGroup != NULL)
    {
        // To jest ta linia, która wywalała błąd:
        xEventGroupSetBits(xEventGroup, BIT_READY_FOR_T1);
    }
    xTaskCreate(task1, "Task1", STACK_SIZE, NULL, osPriorityNormal, NULL);
    xTaskCreate(task2, "Task2", STACK_SIZE, NULL, osPriorityNormal, NULL);
    xTaskCreate(taskPrint, "Print", 1024, NULL, 1,
                &printTaskHandle);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    //
    ////        printf("sample no: %d.\n", tick_us);
    //        if (tick_us % 10 == 0)   xSemaphoreGiveFromISR(sem_100, &xHigherPriorityTaskWoken);
    ////        if (tick_us % P200_US == 0)   xSemaphoreGive(sem_200);
    ////        if (tick_us % P500_US == 0)   xSemaphoreGive(sem_500);
    ////        if (tick_us % P1000_US == 0)  xSemaphoreGive(sem_1000);
    ////        if (tick_us % P5000_US == 0)  xSemaphoreGiveFromISR(sem_5000, &xHigherPriorityTaskWoken);
    ////        if (tick_us % P10000_US == 0) xSemaphoreGiveFromISR(sem_10000, &xHigherPriorityTaskWoken);
    //
