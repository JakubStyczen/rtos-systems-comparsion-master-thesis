...
# ==========================================
    # PLOT 1: DISTRIBUTION (BOXPLOT + STRIPPLOT FOR RTOS)
    # ==========================================
    ...
    is_rtos = os_name in ['Zephyr', 'FreeRTOS']
    
    if is_rtos:
        # --- SCIENTIFIC RTOS PLOT (Boxplot + Swarm/Strip) ---
        # 1. Draw the boxplot but hide the default outliers (fliersize=0)
        ax = sns.boxplot(
        ...
        # 2. Overlay the actual data points to show the exact spread of outliers
        sns.stripplot(
            data=df_plot, x="Target_us", y="Jitter_us", hue="Load",
            dodge=True, alpha=0.3, size=2, color=".2", linewidth=0, ax=ax
        )
        
        # Fix the legend (stripplot duplicates the labels)
        ...
        # --- GPOS PLOT (Violin Plot for Linux/Windows) ---
        ax = sns.violinplot(
            data=df_plot, x="Target_us", y="Jitter_us", hue="Load", 
            split=True, inner="quart", cut=0, density_norm="width",
            palette={"Idle": "#4C72B0", "Stress": "#C44E52"}, linewidth=1.2
        )
        
        # Thicken the quartile lines for GPOS
        for l in ax.lines:
        ...
        plt.legend(title="Load", bbox_to_anchor=(1.02, 0.5), loc='center left')
        plt.title(f"Jitter Distribution vs. Target Timer - {os_name}", fontsize=14, pad=15)

    ax.set_yscale("log")
    ax.set_ylim(bottom=0.05) # Keeps the bottom baseline clean
